...
/**
 * A BayesianNetwork is a network of nodes, each of which corresponds to a
 * random variable.
 * <p>
 * Implementation note: Some aspects of a Bayesian network are typically
 * described as Sets. We maintain the parents of a node as a list since their
 * order determines the way conditional probabilities are managed in the CPT
 * associated with the node. This could, of course, be done otherwise. We
 * maintain child pointers as well as parent pointers simply to make printing
 * easier (not that we bother with graph layout, etc.).
 */
public class BayesianNetwork {

	/**
	 * Inner class representing nodes in a BayesianNetwork. Each node
	 * corresponds to a random variable, and has parent nodes, child nodes, and
	 * a conditional probability table associated with it.
	 */
	protected class Node implements Printable {
		...
		/**
		 * Print this Node to the given PrintWriter at the given indent level.
		 */
	...
	/**
	 * Add a node for the given RandomVariable to this BayesianNetwork. This is
	 * called when a {@code variable} entity is read in an XMLBIF file.
	 */
	...
	/**
	 * Connect the node for the given RandomVariable to the nodes for the given
	 * list of parent RandomVariables, with the given CPT. This is called when a
	 ...
	 * @throws NoSuchElementException
	 *             <p>
	 *             This is currently only used when reading a network from an
	 *             XMLBIF file. If you used it more heavily, you'd probably want
	 *             to index the variables more efficiently.
	 */
	 ...
	 * will be the same as the order they were added to the nextwork.
	 */
	...
	/**
	 * Return the conditional probability that RandomVariable X has the value
	 * assigned to it in the given Assignment, given that its parents have the
	 * values assigned to them in the Assignment.
	 */
	 ...
	 * BayesianNetwork, and return the resulting list. I thought about just
	 * having a {@code topsort} method, into which one would pass a list of
	 * RandomVariables (e.g., from {@link BayesianNetwork#getVariableList}), but
	 * there was just too much converting between RandomVariables and Nodes for
	 * that. The only reason you ever topsort is to topsort the entire list of
	 * variables, so why not just do that here.
	 * <p>
	 ...
	 * RandomVariable. This is done in a really wasteful way, so you'd be better
	 * off doing something different for use in a certain kind of sampling...
	 */
	...
	/**
	 * Print this BayesianNetwork to the given PrintWriter. Let's see. The right
	 * thing is to do a topological sort of the network, which must be a DAG,
	 * then output the nodes in some way that brings out the hierarchy. Or not.
	 */
	public void print(PrintWriter out) {
		for (Node node : nodes) {
