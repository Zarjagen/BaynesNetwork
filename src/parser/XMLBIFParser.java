 ...
 * <a href="http://www.cs.cmu.edu/~fgcozman/Research/InterchangeFormat/">XMLBIF
 * </a> files.
 * <p>
 * Note that XMLBIF explicitly states that
 * <q>There is no mandatory order of variable and probability blocks.</q> This
 * means that we have to read the DOM, then create nodes for all the variables
 * using the {@code variable} elements, then hook them up and add the CPTs using
 * the {@code definition} blocks. A good reason to use a DOM parser rather than
 * a SAX parser.
 * <p>
 * Also XMLBIF appears to use uppercase tag names, perhaps thinking they really
 * ought to be case-insensitive.
 * <p>
 * I have implemented minimal sanity checking and error handling. You could do
 * better. Caveat codor.
 */
public class XMLBIFParser {

	public BayesianNetwork readNetworkFromFile(String filename)
	 ...
	 * succesive probability values of this CPT. This method is used by the
	 * XMLBIFParser, and relies crucially on the ordering of the elements of the
	 * CPT matching the order defined for XMLBIF.
	 * 
	 ...
	 * only looks at the immediate children of type Node.TEXT_NODE or the
	 * children of any child node that is of type Node.CDATA_SECTION_NODE for
	 * the concatenation.
	 */
